...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    std::map<std::string, Weights> weightMap = loadWeights("D:\\Project\\AI_Test\\AI_data\\best_x_sunqt_1116_640x640x1.wts");
...
void doInference(IExecutionContext& context, float* input, float* output, int batchSize, void** buffers, int inputIndex, int outputIndex, cudaStream_t& stream) {
    ...
    CHECK(cudaMemcpy(buffers[inputIndex], input, batchSize * 3 * INPUT_H * INPUT_W * sizeof(float), cudaMemcpyHostToDevice));
...
int main(int argc, char** argv) {
    ...
    for (int f = 0; f < (int)file_names.size(); f++) {
        ...
        doInference(*context, data, prob, BATCH_SIZE, buffers, inputIndex, outputIndex, stream);
        auto end = std::chrono::system_clock::now();
        //add Multi_frames time consuming statistics(Realization)
    ...
    // add Multi_frames time consuming statistics(Realization)
    std::cout << "avg time=" << duration_ms / num << std::endl;
