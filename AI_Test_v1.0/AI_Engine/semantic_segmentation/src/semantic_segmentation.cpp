...
int SemanticSeg::setImage(const cv::Mat &img, bool keep_aspect){
    if (true == keep_aspect){
        ...
        m_img_real_w = img.cols/m_scale_x;
        m_img_real_h = img.rows/m_scale_y;
        ...
        m_scale_x = float(img.cols)/m_net_w;
        m_scale_y = float(img.rows)/m_net_h;
        m_img_real_w = m_net_w;
        m_img_real_h = m_net_h;
        cv::cvtColor(m_img, m_img, cv::COLOR_BGR2RGB);
    }
    return 0;
}

int SemanticSeg::readLabelMap(const string &fileName){
