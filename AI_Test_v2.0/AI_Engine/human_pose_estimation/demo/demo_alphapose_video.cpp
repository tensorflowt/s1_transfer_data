...
int main(int argc, char **argv){
    ...
    parser.about("alphapose_estimation_video");
    if (parser.has("help")){
        parser.printMessage();
        return 0;
    }
    ...
    // 3. deal img
    cv::VideoCapture cap(fileP.c_str()); 
    if(!cap.isOpened()){
        std::cout << "Open video failed"<<std::endl;
        return -1;
    }
    //cv::VideoWriter res("./result.avi", CV_FOURCC('M','J','P','G'), cap.get(CV_CAP_PROP_FPS), 
     //                   cv::Size(cap.get(CV_CAP_PROP_FRAME_WIDTH)*down_rate, cap.get(CV_CAP_PROP_FRAME_HEIGHT)*down_rate)); 
   // if(!res.isOpened()){
    //    std::cout << "Open result video failed"<<std::endl;
     //   return -1;
    //}
    ...
    double selectboxtime = 0.0;
    double imgsettime = 0.0; 
    double poseinfertime = 0.0; 
    double draw_linetime = 0.0;
     ...
     while (true){
        ...
        if (imgM.empty())
            {break;}
        time_end = time_stamp();
        loadtime += (time_end-time_beg)/1000;
        ...
        time_end = time_stamp();
        resizetime += (time_end-time_beg)/1000;
        ...
        time_end = time_stamp();
        detecttime += (time_end-time_beg)/1000;
        ...
        time_end = time_stamp(); 
        selectboxtime += (time_end-time_beg)/1000;

        std::cout<<" Select Bbox time: "<<(time_end-time_beg)/1000<<"ms"<<std::endl;
        // 5. pose img set and infere 
        time_beg = time_stamp();
        for (int j=0; j<allrects.size(); j++){
            cv::Rect rect = allrects[j]; 
            double ste_beg = time_stamp();
            if (0 != estimator->setImage(imgM, rect)){
                std::cout<<"Set Image failed"<<std::endl;
                return -1;
               }
            double ste_end = time_stamp(); 
            imgsettime += (ste_end - ste_beg)/1000;
            std::cout<< "set img time is"<< (ste_end - ste_beg)/1000<<"ms"<<std::endl; 

            double time_beg_infer = time_stamp(); 
            if (0 != estimator->predict()){
                std::cout<<"Predict failed"<<std::endl;
                return -1;
               }
            double time_end_infer = time_stamp();
            poseinfertime += (time_end_infer - time_beg_infer)/1000;
            std::cout<< "one img infer time "<< (time_end_infer - time_beg_infer)/1000<< "ms"<<std::endl; 
            Valpoints points; 
      ...
      //  std::cout<<fileP<<std::endl; 
       // cv::imwrite(fileP, img);
        time_end = time_stamp();
        draw_linetime += (time_end-time_beg)/1000;
    ...
    std::cout<<"load img time: "
             <<loadtime/img_num<<"ms"<<std::endl;
    std::cout<<"resize time: "
             <<resizetime/img_num<<"ms"<<std::endl;
    std::cout<<"detect time: "
             <<detecttime/img_num<<"ms"<<std::endl;
    std::cout<<"select box  time: "
             <<selectboxtime/img_num<<"ms"<<std::endl;
    std::cout<<"img set time: "
             <<imgsettime/img_num<<"ms"<<std::endl;
    std::cout<<"pose infer time: "
             <<poseinfertime/img_num<<"ms"<<std::endl;
    std::cout<<"draw line time: "
             <<draw_linetime/img_num<<"ms"<<std::endl;
